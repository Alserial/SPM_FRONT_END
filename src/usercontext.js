...
export const UserProvider = ({ children }) => {
  ...
  const validate2FAToken = async (username, otp) => {
    try {
      const response = await fetch(backendUrl + "user/validate-otp/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      if (response.ok) {
        // Assuming the backend sends new tokens and user info if the OTP is valid
        localStorage.setItem(
          "userTokens",
          JSON.stringify({
            access_token: data.access_token,
